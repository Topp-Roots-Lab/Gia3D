...
author: Olga Symonova
*/
void PalagyiFilter::applyWdist(hash_map<int, float> &distR)
{
#ifdef DEBUG
			//cout << "PalagyiFilter::apply" << endl;
#endif
  ...
  hash_map<int,pair<int,float>> ndmap;
  hash_map<int,pair<int,float>>::iterator ndit;
  ...
  do {
    ...
    for(int i=0; i < 8; i++) {
	  ...
	  hash_set<int> add_todel;
	  hash_set<int>::iterator ait;
...
*/
int
PalagyiFilter::apply_template_set_std_fast(int direction, hash_set<int> &er, hash_set<int> &bd)
{
  Templates& ts = template_sets[order[direction]];
...
*/
int
PalagyiFilter::apply_template_set_fast(int direction, hash_set<int> &bd)
{
	Templates& ts = template_sets[order[direction]];
...
* by voxels[] array.
*/
int PalagyiFilter::additional_erosion(hash_set<int> &todel, hash_set<int> &bdind)
{
	int Euler=voxels.get_euler_characteristics();
	//printf("Euler characteristics: %d\n",Euler);
	hash_set<int>::iterator bdit;
	VoxelType nb[27];
	int nnb;
	int deleted=0;
